...
Keep limitations, cooldowns, charges, durations, activation actions, and exceptions in limitations or effect descriptions, never in closed-vocabulary fields. Preserve uncertain source details in raw_description and add a review reason rather than replacing them with invented facts. review_kind must be none if and only if review_reasons is empty. visual_ambiguity means the original page image could resolve an OCR-unclear title, number, or closed-vocabulary value; source_ambiguity means the supplied source itself is incomplete, narrative-only, or semantically indeterminate. Do not use visual_ambiguity for source_ambiguity.
