...
func candidateSchema() map[string]any {
	...
	candidate := objectSchema(map[string]any{
		...
		"review_kind":             map[string]any{"type": "string", "enum": []string{"none", "visual_ambiguity", "source_ambiguity"}},
...
Keep limitations, cooldowns, charges, durations, activation actions, and exceptions in limitations or effect descriptions, never in closed-vocabulary fields. Preserve uncertain source details in raw_description and add a review reason rather than replacing them with invented facts. review_kind must be none if and only if review_reasons is empty. Use visual_ambiguity when uncertainty is introduced by OCR or page layout, such as a truncated, broken, or misrecognized title, word, symbol, value, qualifier, condition, or other field, and the original page image can resolve it. Use source_ambiguity only when direct inspection of the original page still cannot establish the source fact because it is incomplete, narrative-only, or semantically indeterminate. Do not use visual_ambiguity for source_ambiguity.
